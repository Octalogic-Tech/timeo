...
const Navbar = ({ reset, setReset, timezone }) => {
  ...
  const handleShare = () => {
    ...
    if (navigator.share) {
      ...
      navigator.clipboard.writeText(
        "Check out what I'm tracking on Timeo! \n" + shareUrl
      );
  ...
  const resetTimezone = () => {
    setReset(!reset);
    dispatch(setBaseTimezone("Asia/Kolkata"));
  };

  return (
    <AppBar style={{ backgroundColor: "#ffffff", color: "#000" }}>
      <Toolbar>
        <Box display="flex" flexDirection="row">
          <Button color="primary" onClick={handleShare}>
            SHARE
          </Button>
          <Button color="primary" onClick={resetTimezone}>
            RESET
          </Button>
        </Box>
        <Box
